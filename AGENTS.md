...
先读 `docs/project/overview.md` 和 `docs/project/rebuild-plan.md`，正式工作按当前 Native change 的 brief、完整目标规格及 Runtime 状态推进。不要用旧项目的代码状态、文档中的已确认标签或旧测试断言替代新版需求。
...
用户已决定旧版功能原则上全部保留，在新版逐项恢复；实现可以重写，阶段后置不等于删除功能。旧账号和业务数据不迁移，新版从空数据开始。用户清空旧数据的意图已记录，实际数据库/存储清理尚未执行，具体目标与范围在后续执行任务中明确。

用户认可旧 UI 设计，要求新版尽量复用旧布局、视觉样式、组件和主要交互。旧界面是优先复用的设计基线；代码实现可以重写，但不能因独立重建默认重设计 UI。必要的界面调整在对应功能规格中说明依据与方案。

首期具体交付范围及各模块规则由对应 Native change 确认。当前整体规划不执行旧库删除；业务代码只在对应 Native change 完成需求确认后实施。

本项目的文档检索主动加入新方向概览和整体规划；旧审查材料不加入新版语料。不得把旧项目测试通过结果写成新版验收结果。
