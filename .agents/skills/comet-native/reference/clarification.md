...
1. 检查是否仍有可能影响结果但尚未明说的假设，并确保 brief 中没有任何 `[blocking]`；
2. 按当前 continuation 执行 `prepare-shape-confirmation`，通过 `--summary` 提交目标、范围、关键决定、验收标准和非目标摘要；不要为最终确认再写一条 `[blocking]`；
3. Runtime 持久化 Shape 确认边界并返回 `await-user` 后，向用户展示完整摘要和 `userCommunication`，然后等待用户明确确认；
4. 用户补充或否定摘要时，更新正式产物，旧确认边界失效并重新准备；
5. 只有用户明确确认后，才执行当前 `commandAlternatives` 中绑定 state version 和 expected action 的 `--confirmed` 动作。
