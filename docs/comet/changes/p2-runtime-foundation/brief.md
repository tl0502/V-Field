...
用户本轮确认：骨架 + 空库 + 最小身份/鉴权；本仓库四套工程；服务端采用旧候选 Node.js + PostgreSQL；数据库使用同机新空库 `vflie`，不清空旧库 `vcar`；正式小程序本轮锁浅色并保留深色源；完成这部分后停下供审查。
...
- 新版 API 连接同主机 `199.85.8.5:5432` 上的新空库 `vflie`（库名可以与旧库不同）。在该库写入本 change 的新空表和预置汽车验证域种子。不迁移旧行。不清空、不改写旧库 `vcar`。不修改旧项目源码、Git 状态或 `.env` 文件。
...
保留新库 `vflie`、旧库 `vcar` 只读、微信端锁浅色与现有 API 地址。本轮代码修复不执行真实库重置、账号初始化或部署。验证使用隔离数据、可注入客户端及本地构建；真实数据库和真机的未跑检查单独记录。修复属于当前身份骨架，不拆分新的业务 change。
...
| S12 | 用户后续：数据库名字可以变；改用新空库 `vflie`，不清空旧库 | complete | 同机新空库 `vflie`；不清空 `vcar`；不改旧项目文件 | Requirement: 同机新空库 vflie | A6 | covered | 主机仍为 `199.85.8.5:5432` |
| B1 | 仓库根现状：无正式 package.json/apps/src/API | complete | 框架规范尚未搭建 | Requirement: 正式工程骨架 | A1 | background | 可调查事实 |
| B2 | 旧库只读调查：`vcar` 现有 13 张 public 表 | complete | 旧库事实；本轮不清空该库 | Requirement: 同机新空库 vflie | A6 | background | 对照 S12，证明旧库未被本 change 删除 |
...
- A6: 新版使用同机新空库 `vflie`，该库只有本 change 的新空表和预置汽车验证域种子；旧库 `vcar` 不被清空；旧项目工作区文件未被修改。
...
- 使用同机新库不等于复用旧表、旧行、旧密钥文件或旧 Runtime，也不等于必须清空旧库。
...
- 已确认（用户）：数据库名字可以改变。新版使用同机新空库 `vflie`（`199.85.8.5:5432`），不清空旧库 `vcar`，不修改旧项目文件。
...
- 核对旧项目文件未被修改；新库 `vflie` 只有新空表和预置汽车验证域种子；旧库 `vcar` 不被本 change 清空。
